...
using Lil.AuthPlatform.AhphOcelot.Helper;
using Ocelot.Cache;
    ...
    public class AhphClientRateLimitProcessor : IClientRateLimitProcessor
    {
        ...
        private async Task<bool> CheckClientReRouteWhiteListAsync(string clientid, string path)
        {
            ...
            {//重新获取限流策略
                ...
                _ocelotCache.Add(key, new ClientRoleModel() { CacheTime = DateTime.Now, Role = result }, TimeSpan.FromSeconds(_option.ClientRateLimitCacheTime), region);
                return result;
            }
        }
        ...
        /// <param name="path">请求地址</param>
        /// <returns></returns>
        private async Task<(bool RateLimit, List<AhphClientRateLimitOptions> rateLimitOptions)>
            CheckClientRateLimitAsync(string clientid, string path)
        {
            ...
            else
            {
                ...
                _rateLimitRuleCache.Add(key,
                    new RateLimitRuleModel() {RateLimit = result.RateLimit, RateLimitOptions = result.rateLimitOptions},
        ...
        {
            var region = _option.RedisKeyPrefix + "CheckReRouteRuleAsync";
            var key = region + path;
            var cacheResult = _ocelotCache.Get(key, region);
            if (cacheResult != null)
            {//提取缓存数据
