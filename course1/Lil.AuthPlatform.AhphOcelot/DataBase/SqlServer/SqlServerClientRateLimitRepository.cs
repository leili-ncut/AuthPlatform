...
namespace Lil.AuthPlatform.AhphOcelot.DataBase.SqlServer
{
    /// <summary>
    /// 客户端限流信息提取
    /// </summary>
    public class SqlServerClientRateLimitRepository : IClientRateLimitRepository
    {
        ...
        public async Task<bool> CheckReRouteRuleAsync(string path)
        {
            ...
            {
                string sql =
                    @"SELECT COUNT(1) FROM AhphReRoute T1 INNER JOIN AhphReRouteLimitRule T2 ON T1.ReRouteId=T2.ReRouteId
        ...
        /// <param name="path">请求地址</param>
        /// <returns></returns>
        public async Task<(bool RateLimit, List<AhphClientRateLimitOptions> rateLimitOptions)>
            CheckClientRateLimitAsync(string clientid, string path)
        {
            using (var connection = new SqlConnection(_option.DbConnectionStrings))
            {
                string sql =
                    @"SELECT DISTINCT UpstreamPathTemplate AS RateLimitPath,LimitPeriod AS Period,LimitNum AS Limit,ClientId FROM AhphReRoute T1 INNER JOIN AhphReRouteLimitRule T2 ON T1.ReRouteId=T2.ReRouteId
                ...
                var result = (await connection.QueryAsync<AhphClientRateLimitOptions>(sql, new {clientid, path}))
                    ?.AsList();
        ...
        public async Task<bool> CheckClientReRouteWhiteListAsync(string clientid, string path)
        {
            ...
            {
                string sql =
                    @"SELECT COUNT(1) FROM AhphReRoute T1 INNER JOIN AhphClientReRouteWhiteList T2 ON T1.ReRouteId=T2.ReRouteId
